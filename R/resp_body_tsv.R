...
#' @param page `integer` (optional), response page number. If `page` > 1 then
#'   the table header is removed before saving to file. Only used when `con` is
#'   specified.
#' @param con `string` or [base::connection] object (optional), the file in which
#'   to save the data.
#' @param encoding `string` (optional), character encoding of the body text. If
#'   not specified, will use the encoding specified by the content-type, falling
#'   back to `"UTF8"` with a warning if it cannot be found. The resulting string
